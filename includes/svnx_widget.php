...
class bookx_widget {
    ...
    */
    ...
    function bookx_widget_sidebar($args){
        ...
        if (substr_count($this->options["widgetTemplate"], "::IMAGE::")){
            $doImage = true;            
        }
        ...
        foreach($results as $row){
            if ($doImage){
                ...
                if ($sourceWidth > $this->options["widget_image_width"]) {
                    $newWidth  = $this->options["widget_image_width"];
                    $newHeight = (INTEGER) ($sourceHeight * ($this->options["widget_image_width"] / $sourceWidth));
                } 
                else if ($sourceHeight > $this->options["widget_image_height"]) {
                    $newWidth  = (INTEGER) ($sourceWidth * ($this->options["widget_image_height"] / $sourceHeight));
                    $newHeight = $this->options["widget_image_height"];
                } 
                else {
            ...
            $text .= "<li>";
            $text .= strtr(stripslashes($this->options["widgetTemplate"]), $trans);
    ...
    */
    ...
    function bookx_widget_admin(){
        if ($_POST["bookx_submit"]){
            $this->options["widget_title"]          = $_POST["bookx_title"]; 
            $this->options["widget_image_height"]   = $_POST["bookx_height"];
            $this->options["widget_image_width"]    = $_POST["bookx_width"];
            $this->options["widget_sort"]           = $_POST["bookx_sort"];
            $this->options["widget_order"]          = $_POST["bookx_order"];
            update_option('bookx_options', $this->options);    
        }
        
        
        $text  = "<strong>Title:</strong> <input type=\"text\" name=\"bookx_title\" value=\"" . $this->options["widget_title"] . "\" /><br />";
        $text .= "<strong>Max Image Size:</strong> <input type=\"text\" name=\"bookx_height\" size=\"3\" value=\"" . $this->options["widget_image_height"] . "\" /> x ";
        $text .= "<input type=\"text\" name=\"bookx_width\" size=\"3\" value=\"" . $this->options["widget_image_width"] . "\" /><br />";
        $text .= "<strong>Order on:</strong> <select name=\"bookx_order\">";
        foreach(array_keys($this->fieldArray) as $f){
            if ($f == $this->options["widget_order"]){ $s = "selected"; }
            else { $s = ''; }
        ...
        foreach(array_keys($this->sortArray) as $sort){
            if ($sort == $this->options["widget_sort"]){ $s = "selected"; }
            else { $s = ''; }
